...
export function StickyCta({ signedIn }: { signedIn: boolean }) {
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-hairline bg-parchment/95 backdrop-blur-md sm:hidden">
      <div className="flex items-center gap-4 px-6 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
        <p className="flex-1 text-fine text-ink-48">ใช้เวลา 2–3 นาที ไม่มีค่าสมัคร</p>
        <AuthCta
          signedIn={signedIn}
          href="/apply"
          className="min-h-[44px] px-5 text-caption"
        >
          สมัครเลย
        </AuthCta>
      </div>
